...
try:
    ...
    # Simple fallback for aiofiles
    class aiofiles:
        @staticmethod
        def open(path, mode='r'):
            return AsyncFileWrapper(path, mode)
...
class AsyncFileWrapper:
    ...
    def fileno(self):
        return self.file.fileno() if self.file else None
...
try:
    ...
    # Mock frontmatter if not available
    class frontmatter:
        @staticmethod
        def Post(content, metadata=None):
            return type('Post', (), {'content': content, 'metadata': metadata or {}})()
